...
function check_user_login_info($username, $user_key){
	...
	$sql = "SELECT username, password FROM admin WHERE username= '". secure_sql($username) ."'";
...
function confirm_login( $username, $pass, $hashed = false ){
	...
	$sql = "SELECT id, username, password, power FROM admin WHERE username= '". secure_sql($username) ."'";
	$result = @mysql_query($sql);
	if( !$result ) 
		return 0;
	$rows = @mysql_num_rows($result) ;
...
function reset_password($email_address = ''){
	...
	$sql = "UPDATE admin SET new_password = '".$new_md5."', activation_key = '".$activation_key."' WHERE email= '".$email_address."'";
